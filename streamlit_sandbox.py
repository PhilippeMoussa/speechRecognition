...
if page==pages[4]:
    ...
    ##  model5 = keras.models.load_model('/content/drive/My Drive/spych/v12big/h5model.h5')

    ###
    
        record = st.file_uploader("upload audio file")
        #filePath = 'samples/'
        col1, col2 = st.columns(2)
        with col1:
            st.audio(record)
        with col2:
            st.button("predict", on_click = predict(record.name)) 
...
if page==pages[5]:
    ...
    with st.container():
        ...
        with col1:
            ...
            - Ajouter des couches à notre modèle
            """)
        with col2:
            st.markdown('##')
